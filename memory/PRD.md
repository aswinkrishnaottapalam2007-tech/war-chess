...
## Studio opening, live voice, and publishing hardening — development follow-up
- User requested: "solve every problem and make the app good for publishing. use the file uploaded and make it as the app opening it's the studio logo."
- Explicit choices: opening once per app launch with Skip; live voice must work without requesting further external-service credentials.
- Bundled complete CHAOS ENGINE STUDIO clip (actual duration about 10 seconds), optimized H.264/AAC for native plus WebM for browser compatibility, contain fit, skip/sound controls, error/timeout escape, no replay on menu navigation. Original source is the user's uploaded customer-assets MP4. Replaced generic icon/splash with managed-hosted WAR CHESS king artwork.
- Replaced credential-dependent LiveKit flow with actual self-hosted WSS live audio relay: 16kHz mono PCM, 80ms bounded frames, transient RAM-only fan-out, bounded jitter/backpressure, six-player room limit, microphone mute/leave, participant roster. Root-owned voice persists while the chat sheet is closed; native WebView host uses contextual microphone permissions and the same audio processing code.
- Voice is encrypted in transit, NOT end-to-end encrypted, and foreground-only. Per-room worker ownership leases prevent split local voice rooms; multiple workers require room-sticky routing. Current managed backend uses one worker. Real-device audio must still be verified.
- Security fixes implemented pending independent tests: server-driven session/membership sweeps and per-broadcast revalidation; logout immediately closes matching local sockets; distributed per-user socket slots, admission/action limits, voice single-use tickets tied to parent sessions, no public leaderboard internal UUIDs, minimal public health with authenticated diagnostics, and API security headers.
- Added in-app account deletion with password/DELETE confirmation, immediate access invalidation, persisted cleanup/retry, chat/statistics removal and match anonymization; added privacy/voice information page.
- Preserved saved sessions during transient /auth/me network failures instead of silently deleting tokens.
- TypeScript and Python/JS lint checks pass. Independent full verification is the next gate; do not mark these security fixes or new voice flow verified until that report returns.
- Iteration_5 independently passed new voice ticket/PCM relay contracts, health/leaderboard privacy, and deletion API; browser natural intro completion/Skip/reload worked but exposed play/pause Promise noise and sheet reachability issues. Those UI paths have been revised for retesting (caught browser playback promises; explicit viewport-bounded modal layout).
- Diagnostic evidence showed the public managed proxy rewrites WebSocket Origin to an internal same-host origin. Added browser Fetch Metadata rejection for cross-site sockets; original Origin validation remains where preserved. Bad-Origin tests must distinguish direct application enforcement from proxy-normalized headers. Logout test also needs to distinguish already-buffered pre-logout ping replies from new protected data; revised independent tests requested, not treated as verified yet.
- Iteration_6 independently passed **21/21 backend checks** (12 security/voice, 9 core API), and real two-client browser capture/relay/playback with synthetic microphone input. Verified roster, mute/unmute, leave, active voice after chat closure, intro visible playback/natural finish/Skip/no in-app replay, and account deletion with subsequent login denial. Report: test_reports/iteration_6.json. No application APIs were mocked. Native microphone hardware remains unverified.
- Corrected SEC001 test sequencing confirmed immediate logout close, server expiry sweep, authenticated connection caps, direct Origin denial, and public browser cross-site denial. The earlier two critical failures were test/proxy interpretation artifacts, not evidence of post-fix unauthorized state delivery.
- Final cleanup removes unused LiveKit client/API dependencies and legacy bundle, moves owned board pointer handling to the supported style property, and records visited rooms so account deletion also removes chat/history from rooms no longer joined.