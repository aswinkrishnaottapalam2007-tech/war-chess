...
export function Sheet({ visible, title, onClose, children }: { visible: boolean; title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modal}>
    <Pressable accessibilityLabel="Close sheet" testID="sheet-backdrop" style={s.backdrop} onPress={onClose} />
    <View testID="action-sheet" style={[s.sheet, { height: Math.min(height - insets.top - 20, Math.max(height * 0.78, 540)), paddingBottom: Math.max(insets.bottom, 20) }]}><View style={s.handle} /><View style={s.sheetHeader}><Text style={s.sheetTitle}>{title}</Text><Pressable testID="close-sheet-button" accessibilityLabel="Close" onPress={onClose} style={s.iconButton}><Ionicons name="close" size={24} color={colors.onSurface} /></Pressable></View><ScrollView style={s.sheetScroll} keyboardShouldPersistTaps="handled" contentContainerStyle={s.sheetContent}>{children}</ScrollView></View>
